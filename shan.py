...
def gaussian_noise (m, n):
	...
	array = numpy.multiply(array, 1.5)
...
def generatePSF(image):
	...
	# PSF = cv2.circle(PSF, center_coordinates, 2, (255), -1)
...
img = cv2.imread('shan.png', 0)
img_arr = numpy.array(img)

cv2.imshow('image', img_arr)
cv2.waitKey(0)

colour_image = cv2.imread('shan.png', 1)
colour_image_array = numpy.array(colour_image)

colour_image_luminance = RGB_to_luminance(colour_image_array)


constant = img_arr[ img_arr.shape[0] - 50 : img_arr.shape[0], img_arr.shape[1] - 50 : img_arr.shape[1]]
...
# Gaussian noise with mean = 0 and standard deviation = 5/3


point = img_arr[ 300 : 325, 675 : 700]
...
plt.hist(point, bins = 10, density = True)
plt.show()

# Point Spread Function for motion blur can be assumed as gaussian filter or disk shaped filter
...
median = cv2.medianBlur(recovered_image, 3)

cv2.imshow('median', median)
cv2.waitKey(0)

mean = cv2.blur(recovered_image, (3,3))
...
sharpened = cv2.filter2D(img_arr, -1, kernel_sharpening)
